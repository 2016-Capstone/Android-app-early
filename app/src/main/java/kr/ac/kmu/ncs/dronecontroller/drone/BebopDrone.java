package kr.ac.kmu.ncs.dronecontroller.drone;
...
public class BebopDrone {
    ...
    public interface Listener {
        ...
        void onFrameReceiver(ARFrame frame);

        //  Called before medias will be downloaded
        void onMatchingMediasFound(int nbMedias);

        //  Called each time the progress of a download changed
        void onDownloadProgressed(String mediaName, int progress);

        //  Called when a media donwload has ended
    ...
    Constructor
     */
    public BebopDrone(Context context, ARDiscoveryDeviceService deviceService) {
        ...
        if (ARDISCOVERY_PRODUCT_FAMILY_ENUM.ARDISCOVERY_PRODUCT_FAMILY_ARDRONE.equals(family)) {
            ...
            try {
                String productID = ((ARDiscoveryDeviceNetService) (deviceService.getDevice())).getIp();

                ARUtilsManager ftpListManager = new ARUtilsManager();
                ARUtilsManager ftpQueueManager = new ARUtilsManager();

                ftpListManager.initWifiFtp(productID, DEVICE_PORT, ARUtilsFtpConnection.FTP_ANONYMOUS, "");
                ftpQueueManager.initWifiFtp(productID, DEVICE_PORT, ARUtilsFtpConnection.FTP_ANONYMOUS, "");

                mSDCardModule = new SDCardModule(ftpListManager, ftpQueueManager);
                mSDCardModule.addListener(mSDCardModuleListenener);
     ...
     */
    public ARCOMMANDS_ARDRONE3_PILOTINGSTATE_FLYINGSTATECHANGED_STATE_ENUM getmFlyingState() {
        return mFlyingState;
    }
    ...
    /*
    Set the sid angle of the drone
     */
    ...
    private ARDiscoveryDevice createDiscoveryDevice(ARDiscoveryDeviceService service, ARDISCOVERY_PRODUCT_ENUM productType) {
    ...
    private ARDeviceController createDeviceController(ARDiscoveryDevice discoveryDevice) {
    ...
    private void notifyBatteryChanged(int battery) {
        List<Listener> listenersCpy = new ArrayList<>(mListeners);
        for (Listener listener : listenersCpy) {
            listener.onBatterChargeChanged(battery);
        }
    }
    ...
    private void notifyFrameReceived(ARFrame frame) {
        List<Listener> listenersCpy = new ArrayList<>(mListeners);
        for (Listener listener : listenersCpy) {
            listener.onFrameReceiver(frame);
        }
    }
    ...
    private final SDCardModule.Listener mSDCardModuleListenener = new SDCardModule.Listener() {
        @Override
